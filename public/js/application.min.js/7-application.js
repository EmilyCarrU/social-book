...
var apiEndpoint = 'http://dev.dotkenneth.com/hyko/api.php/api';

Offline.options = {checks: {xhr: {url: apiEndpoint}}};

window.App = {};
window.App.online = Offline.state;
     ...
     $.ajax({
       ...
       complete: function(xhr, status) {
         ...
         if (response.status === "error") {
           if (response.status === "Please include all required arguments (name, email, content).") {
           }
       ...
       error: function(xhr, status) {
         console.log(xhr)
         alert("Sorry, there was a problem. Please try again later or send Adam an email.")
       }
     })
    }
  }
...
App.SubChapterView = Backbone.View.extend({
  ...
  test: function(e) {
    ...
    // Since target it an overlay, this prevents you from doing anything
    // inside the comment area
  ...
  addOne: function(comment) {
    ...
    this.$el.find('.comment-list').append('<li class="comment topcoat-list__item new-item"><div class="comment-content"><div class="comment-body">' + comment.attributes.content + '</div><header class=""><strong class="comment-author">&mdash;' + comment.attributes.name + '</strong></header></div></li>');
...
App.CommentsView = Backbone.View.extend({
  ...
  render : function() {
    ...
    var template2 =  _.template($("#template-comment-form").html());
    var html2 = template2();
    $(this.el).find('.dive').append(html2);
